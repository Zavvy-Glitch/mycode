...
def main():
    ...
    # the player known as mulligan
    mulligan = Mulligan_Cheater()
    ...
    while game_number < number_of_games:
        ...
        if sum(swapper.get_dice()) == sum(loaded_dice.get_dice()) == sum(mulligan.get_dice()):
            print("Draw!")
            pass
        elif sum(swapper.get_dice()) > sum(loaded_dice.get_dice()) and sum(mulligan.get_dice()):
            print("Dice swapper wins!")
            swapper_score += 1
        elif sum(mulligan.get_dice()) > sum(swapper.get_dice()) and sum(loaded_dice.get_dice()):
            print("Mulligan wins!")
            mulligan_score += 1
        else:
    ...
    if swapper_score == loaded_dice_score == mulligan_score:
        print("Game was drawn")
    elif swapper_score > loaded_dice_score and mulligan_score:
        print("Swapper won most games")
    elif mulligan_score > swapper_score and loaded_dice_score:
